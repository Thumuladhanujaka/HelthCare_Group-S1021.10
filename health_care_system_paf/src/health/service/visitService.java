...
private int success;
	...
	public void addVisits(Visit visit) {
		...
		try {
			...
			//insert value
			preparedStatement = connection.prepareStatement("insert into visit (name,date,time,hospital,pack) values (?,?,?,?,?)");
			...
			preparedStatement.setString(5,visit.getPack());
	...
	public ArrayList<Visit> getVisit() {
		...
		try {
			...
			while (resultSet.next()) {
				...
				visit.setPack(resultSet.getString(6));
				visit.setHname(resultSet.getString(7));
	...
	public Visit getVisit(int id) {
		...
		try {
			...
			while(rs.next())
			{
				...
				visit.setHname(rs.getString(7));
				visit.setPack(rs.getString(6));
			}
	...
	public String getPacks(int id) {
		Connection connection;
		PreparedStatement preparedStatement;
		String code = "<select name='packages' class='form-control' id='packages'><option value=''>Select</option>";
		
		try {
			connection = DBConnect.getDBConnection();
			
			preparedStatement = connection.prepareStatement("select * from payment WHERE hospital=?");
			preparedStatement.setInt(1, id);
			ResultSet rs = preparedStatement.executeQuery();
			...
			while(rs.next())
			{
				test=test+"<option value='"+"Name : "+rs.getString(2)+" / Price : "+rs.getDouble(5)+"'> Name : "+rs.getString(2)+" / Price : "+rs.getDouble(5)+"</option>";
			}
